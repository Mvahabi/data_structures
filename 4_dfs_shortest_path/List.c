...
bool equals(List A, List B) {
    // three base cases here:
    // check if A and B exist
    // check if A or B exist
    // check if the lengths of A and B are similar
    // have two temp nodes, both assigned to the front of both lists
    // iterate through a list
        // check to see if the data's of the two nodes are similar
        // if so, continue by moving the nodes forward
        // if not exit the loop
    // if the for loop ran successfully then you return true
}
